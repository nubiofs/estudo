...
{mdpi_com_adserver_banners[id]["counter"]+=1;mdpi_com_adserver_banners[id]["same-threshold"]-=1;if(mdpi_com_adserver_banners[id]["counter"]<5)
...
{var div_animation=div.find(".animation-content");if(div_animation.length>0){div_animation.append('<div style="position:relative; display: block; float: left; padding:0; margin:0">'+content+'</div>');var slideWidth=div_animation.closest('.adserver-banner').width();div_animation.find('img').css('width',slideWidth+'px');if(div_animation.children("div").length>1)
{mdpi_com_adserver_banners[id]["same-threshold"]=-1;div_animation.stop().animate({left:-slideWidth},function()
{div_animation.children("div:first-child").remove();$(this).css('left','');});}}
else{div.append(content);}
mdpi_com_adserver_banners[id]["counter"]=0;mdpi_com_adserver_banners[id]["current"]=matches[2];mdpi_com_adserver_banners[id]["rotate_counter"]+=1;}
else
{clearInterval(mdpi_com_adserver_banners[id]["interval"]);}}}}}});}}
...
{var div=$(this);var id=div.attr("id");var max_rotation=div.attr("max-rotation")==undefined?50:div.attr("max-rotation");if(id==undefined)
{console.log("Banner initialization error: unique banner div id *must* be defined");}
else
{mdpi_com_adserver_banners[id]={};mdpi_com_adserver_banners[id]["current"]="-1";mdpi_com_adserver_banners[id]["counter"]=0;mdpi_com_adserver_banners[id]["rotate_counter"]=0;mdpi_com_adserver_banners[id]["same-threshold"]=default_threshold;if(div.attr("value")==undefined)
...
return moment;}
var getDuration=function(service,level){var m=moment();if("layout"===service||"both"===service){m=momentAddDays(m,1,false);result="Editing will be completed on "+m.format("D MMMM");}
...
return result;}
var get_price=function(word_num,has_manuscript,service,level){var price=0;if("layout"===service||"both"===service){price+=75;}
switch(level){case "regular":price=(price+105+word_num*0.02).toFixed(2);break;case "express":price=(price+175+word_num*0.025).toFixed(2);break;case "specialist":price=(price+180+word_num*0.04).toFixed(2);break}
return price;};var getEncodedParamsString=function(word_num,service,level,subject)
{var params="num="+word_num+"&service="+service;if(null!==level){params+="&level="+level;}
if(null!==subject){params+="&subject="+subject;}
return encodeURIComponent(base64_encode(params));}
var updateSubmitLink=function(submit)
{var price;var word_num=parseInt($('#word_num').val(),10);var id=$('#manuscript_id').val().match(/\d+/);var service=$("input:radio[name='service']:checked").val();var level=null;var subject=null;if(id){id=id[0];}
if("english"===service||"both"===service){level=$("input:radio[name='level']:checked").val();}
if("specialist"===level){subject=$("#specialist-subject").val();}
var manuscript_id=parseInt(id,10);var susy_host=$('#susy_host').val();if(word_num>0){if(manuscript_id>0){$.get('/is_under_review/'+manuscript_id,function(data){var has_manuscript=data.exist;var price=get_price(word_num,has_manuscript,service,level);$("#price").html(price);$("#duration").html(getDuration(service,level));var params=getEncodedParamsString(word_num,service,level,subject);if(!has_manuscript){$('#msg').html('No manuscript found!').show();$('#proceed_btn').data('href',susy_host+'user/pre_english/upload'+"?params="+params);}
...
else{var price=get_price(word_num,false,service,level);$("#price").html(price);$("#duration").html(getDuration(service,level));var params=getEncodedParamsString(word_num,service,level,subject);$('#proceed_btn').data('href',susy_host+'user/pre_english/upload'+"?params="+params);if(submit){window.location.href=$('#proceed_btn').data('href');}}}}

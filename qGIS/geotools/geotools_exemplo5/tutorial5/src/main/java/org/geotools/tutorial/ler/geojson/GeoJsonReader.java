...
public class GeoJsonReader {
	...
	public List<Geometry> readListGeometry(String json) throws ParseException {
		List<Geometry> result = readListGeometry(new StringReader(json));
		return result;
	}
	...
	public List<Geometry> readListGeometry(Reader reader) throws ParseException {
		...
		try {
			...
			result = createListGeometry(geometryMap, geometryFactory);
	...
	private Geometry createGeometry(Map<String, Object> featuresMap,
			GeometryFactory geometryFactory) throws ParseException {

		Geometry result = null;

		String type = (String) featuresMap.get(GeoJsonConstants.NAME_TYPE);
		...
		if (type == null) {
			...
			if ("FeatureCollection".equals(type)) {
				...
				JSONArray features =  (JSONArray) featuresMap.get("features");
				...
				if(features.size() == 1){
					...
					@SuppressWarnings("unchecked")
					Map<String, Object> geometryMap = (Map<String, Object>) geo;
	...
	private List<Geometry> createListGeometry(Map<String, Object> featuresMap,
			GeometryFactory geometryFactory) throws ParseException {

		List<Geometry> retorno = new ArrayList<Geometry>();

...
show_help() {
   ...
   echo "Exemplo: infra-batch.sh -c /opt/infra-batch -l /opt/logs xxx-batch.jar -x jobs/MeuJob.xml -i jobId -p \"parametroX=XXX parametroY=\\\"YY YY\\\"\""
   echo "------------*------------"
}
...
then
	...
	echo "EXEMPLO: -jar xxx-batch.jar"	
	echo "####################################################################################"
	exit 1
fi
...
echo "INFRA-BATCH script v$INFRA_BATCH_SCRIPT_VERSION"
echo "executando..."
echo "java $JAVA_OPTS -Dinfra.config.dir=$CONFIG_PATH -Dinfra.log.dir=$LOG_PATH $ENV $VERBOSE -jar $CONFIG_PATH/$JAR $XML $ID $PARAMS"

java $JAVA_OPTS -Dinfra.config.dir=$CONFIG_PATH -Dinfra.log.dir=$LOG_PATH $ENV $VERBOSE -jar $CONFIG_PATH/$JAR $XML $ID $PARAMS

#Pastas em homologação
#/opt/approtinas/hom_xxxbatch

#/opt/appfiles/hom_xxxbatch/entrada
